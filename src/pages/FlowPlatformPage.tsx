...
import { PersonaTabs } from '@/components/flow/PersonaTabs';
...
interface FlowPlatformPageProps {
  platform: Platform;
}

/** Flow-only platform page: product cards + persona-tabbed mockups. */
export default function FlowPlatformPage({ platform }: FlowPlatformPageProps) {
  const memberProducts = getPlatformProducts(platform);

  return (
    <>
      <Section bg="dark" py={{ xs: 7, md: 9 }} id="flow-hero">
      ...
      <Section bg="light" py={{ xs: 4, md: 5 }} id="flow-stats">
        <Container maxWidth="lg">
          <StatRow stats={platform.combinedStatRow} animate />
        </Container>
      </Section>
      ...
      <Section bg="alt" id="by-persona">
        <Container maxWidth="lg">
          <SectionHeading
            eyebrow="By persona"
            title="One Flow — three operating views"
            subtitle="Procurement, finance, and distributor operations each see the same platform through the outcomes they own."
          />
          <PersonaTabs />
        </Container>
      </Section>
