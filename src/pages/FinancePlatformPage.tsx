...
import { OurJourneyTimeline } from '@/components/finance/OurJourneyTimeline';
import { MoneyFlowHero } from '@/components/finance/MoneyFlowHero';
...
interface FinancePlatformPageProps {
  platform: Platform;
}

/** Finance-only platform page with journey timeline, count-up stats, and money-flow hero. */
export default function FinancePlatformPage({ platform }: FinancePlatformPageProps) {
  const memberProducts = getPlatformProducts(platform);
  const heroScrollRef = useRef<HTMLElement | null>(null);

  return (
    <>
      <Section bg="dark" py={{ xs: 7, md: 9 }} id="finance-hero" ref={heroScrollRef}>
      ...
      <Section bg="light" py={{ xs: 4, md: 5 }} id="finance-stats">
        <Container maxWidth="lg">
